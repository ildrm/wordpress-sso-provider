...
Enable sodium/OpenSSL and set a dedicated `WP_SSO_MASTER_KEY` of at least 32
...
exact absolute redirect URI, the URI uses HTTPS (except loopback development), and
scopes are from `openid profile email roles groups`.
